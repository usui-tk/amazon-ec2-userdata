...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/refs/heads/main/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_AmazonLinux-2027-HVM.json"

#-------------------------------------------------------------------------------
# Acquire unique information of Linux distribution
#  - Amazon Linux 2027
#    https://aws.amazon.com/jp/linux/amazon-linux-2027/
#    https://aws.amazon.com/jp/linux/amazon-linux-2027/#ams%23rt-groupable-faqc9%23pattern-data
#
#    https://docs.aws.amazon.com/linux/index.html
#    https://docs.aws.amazon.com/linux/al2027/release-notes/relnotes.html
#    https://docs.aws.amazon.com/linux/al2027/ug/what-is-amazon-linux-2027.html
#
#    https://github.com/amazonlinux/amazon-linux-2027
#
#-------------------------------------------------------------------------------
...
dnf install -y dnf dnf-utils "dnf-plugins-*"
...
dnf install -y 7zip-standalone-all acpid arptables atop bash-completion bc bcc bcc-tools bind-utils blktrace bpftool bpftrace bzip2 collectd collectd-utils crypto-policies dmidecode dnf-plugins-core dnf-utils ebtables ethtool expect fio fping gdisk git git-lfs gnutls-utils hdparm htop icu iftop inotify-tools intltool iotop ipcalc iperf3 iproute-tc ipset jq kexec-tools libbpf-tools libicu libzip-tools low-memory-monitor lsof lsscsi lvm2 lzop man-pages mc mdadm mtr nc ncompress net-snmp-utils net-tools nftables nkf nmap nmap-ncat numactl nvme-cli parted patchutils perf psacct psmisc rsync screen smartmontools strace symlinks sysfsutils sysstat tcpdump time traceroute tree tzdata unzip util-linux util-linux-user uuid vim-enhanced wget wireshark-cli xfsdump xfsprogs yq yum-utils zip zsh zstd

# Package Install NFS/CIFS Administration Tools (from Amazon Linux Official Repository)
dnf install -y cifs-utils nfs-utils nfsv4-client-utils nfs-stats-utils nfs4-acl-tools
...
dnf install -y awscli-2 amazon-chrony-config amazon-ec2-net-utils amazon-linux-repo-s3 amazon-linux-sb-keys amazon-rpm-config amazon-ssm-agent aws-cfn-bootstrap aws-lc-libs cloud-init-cfg-ec2 dracut-config-ec2 ec2-instance-connect ec2-instance-connect-selinux ec2-utils
...
# https://docs.aws.amazon.com/linux/al2027/ug/live-patching.html
#-------------------------------------------------------------------------------
...
# dnf --repo="amazonlinux-spal" list --available > /tmp/command-log_dnf_repository-package-list_amazonlinux-spal.txt
...
if [ -n "$RoleName" ]; then
	...
	LatestAmiId=$(aws ec2 describe-images --owner "amazon" --filter "Name=name,Values=al2027*" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=x86_64" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)' --output json --region ${Region} | jq -r '[.[] | select(contains({Name: "minimal"}) | not)] | .[0].ImageId')
...
# curl -fsSL https://fluentd.cdn.cncf.io/sh/install-amazon2027-fluent-package6-lts.sh | sh
...
# dnf --repo="hashicorp" list --available > /tmp/command-log_dnf_repository-package-list_hashicorp.txt
