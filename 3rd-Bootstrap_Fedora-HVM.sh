...
dnf install -y abrt abrt-cli acpid aria2 arptables atop bash-completion bc bcc bcc-tools bcftools bind-utils blktrace bpftool bpytop byobu collectd collectd-utils collectl colordiff colorized-logs console-login-helper-messages-motdgen crash-trace-command crudini crypto-policies curl dateutils dnf-data dnf-plugins-core dnf-utils dstat ebtables ethtool expect fio fping fzf gdisk git git-lfs glances gnutls-utils hdparm htop httping icu iftop inotify-tools intltool inxi iotop ipcalc iperf3 ipset iptraf-ng ipv6calc java-latest-openjdk jc jq kexec-tools libicu linuxptp lsof lvm2 lzop man-pages mc mcelog mdadm moreutils moreutils-parallel mtr nc ncdu ncompress net-snmp-utils net-tools nftables nkf nload nmap nmap-ncat numactl nvme-cli nvmetcli parted patchutils pmempool psacct psmisc python3-dnf-plugin-versionlock rsync screen smartmontools sos srm ssh-audit sslscan strace stressapptest symlinks sysfsutils sysstat tcpdump time tlog tmpwatch traceroute tree tzdata unzip usermode util-linux util-linux-user uv vim-enhanced vnstat wdiff wget wireshark-cli xfsdump xfsprogs xq yamllint yq yum-utils zip zsh zstd
