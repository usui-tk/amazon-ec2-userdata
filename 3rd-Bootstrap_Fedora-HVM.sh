...
dnf install -y abrt abrt-cli acpid aria2 arptables atop bash-completion bc bcc bcc-tools bcftools bind-utils blktrace bpftool bpytop byobu collectd collectd collectd-utils collectd-utils collectl colordiff colorized-logs console-login-helper-messages-motdgen crash-trace-command crudini crypto-policies curl dateutils dnf-data dnf-plugins-core dnf-utils dstat ebtables ethtool expect fio fping fzf gdisk git glances gnutls-utils hdparm htop httping iftop inotify-tools intltool inxi iotop ipcalc iperf3 ipset iptraf-ng ipv6calc jc jq kexec-tools libicu linuxptp lsof lvm2 lzop man-pages mc mcelog mdadm moreutils moreutils-parallel mtr nc ncdu ncompress net-snmp-utils net-tools nftables nkf nload nmap nmap-ncat numactl nvme-cli nvmetcli parted patchutils pmempool psacct psmisc python3-dnf-plugin-versionlock rsync screen smartmontools sos srm ssh-audit strace stressapptest symlinks sysfsutils sysstat tcpdump time tlog tmpwatch traceroute tree tzdata unzip usermode util-linux util-linux-user vim-enhanced wdiff wget wireshark-cli xfsdump xfsprogs yamllint yq yum-utils zip zsh zstd
