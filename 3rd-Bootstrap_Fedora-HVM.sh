...
if [ -n "$RoleName" ]; then
	...
	NewestAmiInfo=$(aws ec2 describe-images --owner "125523088429" --filter "Name=name,Values=Fedora-Cloud-Base-*-gp2*" "Name=virtualization-type,Values=hvm" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)|[0]' --output json --region ${Region})
