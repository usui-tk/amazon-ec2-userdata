...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/master/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_RockyLinux-v9-HVM.json"
...
#-------------------------------------------------------------------------------
# Yum Repositories (Rocky Linux v9)
...
# Package Install Rocky Linux yum repository Files (from Rocky Linux Repository)
...
# Package Install Rocky Linux Linux-Kernel Modules (from Rocky Linux Repository)
dnf install -y kernel-modules kernel-modules-extra

# Package Install Rocky Linux System Administration Tools (from Rocky Linux Repository)
...
dnf --enablerepo="epel" install -y atop bash-color-prompt byobu collectd collectd-utils colordiff dateutils fping glances htop iftop inotify-tools inxi ipv6calc jc lsb_release moreutils moreutils-parallel ncdu nload screen stressapptest unicornscan wdiff yamllint

# dnf --enablerepo="epel" install -y atop bash-color-prompt bcftools bpytop byobu collectd collectd-utils colordiff dateutils fping glances htop httping iftop inotify-tools inxi ipv6calc jc jnettop lsb_release moreutils moreutils-parallel ncdu nload screen srm stressapptest tcping unicornscan wdiff yamllint
...
if [ -n "$RoleName" ]; then
	...
	NewestAmiInfo=$(aws ec2 describe-images --owner "679593333241" --filter "Name=name,Values=Rocky-9-EC2-9.*" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=x86_64" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)|[0]' --output json --region ${Region})
...
# Package Install Tuned (from Rocky Linux Repository)
