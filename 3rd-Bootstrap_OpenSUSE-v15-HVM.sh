...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/refs/heads/main/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_OpenSUSE-HVM.json"
...
zypper --quiet --non-interactive install aaa_base aaa_base-extras aria2 arptables atop bash-completion bash-loadables bc bcc-tools bind-utils blktrace cloud-netconfig-ec2 collectl conntrack-tools curl dstat ebtables ethtool expect fio gdisk git-core glib2-lang glibc-extra gperftools hdparm hostinfo intltool iotop iotop jq kexec-tools kmod-bash-completion libicu lsb-release lvm2 lzop man-pages man-pages-posix mcelog mdadm mlocate mtr net-snmp nftables nmap numactl nvme-cli nvmetcli parted patchutils pmdk pmdk-tools psmisc psutils rsync sdparm seccheck smartmontools ssh-audit strace supportutils sysfsutils syslinux sysstat system-group-wheel system-user-bin system-user-daemon tcpdump time traceroute tree tuned unrar unzip util-linux vim-enhanced wget xfsdump xfsprogs yq zip zypper-log
