...
#-------------------------------------------------------------------------------
dnf install -y java-11-openjdk java-11-openjdk-devel java-17-openjdk-headless
...
# Identification of DSS (v13) version
curl -s https://cdn.downloads.dataiku.com/public/dss/ | grep -oP '(?<=href=")[^"]+' | grep -E '^([0-9]+\.)+[0-9]+/$' | sort -V | grep -ie "13.*"

# Latest version of DSS (v13) version
DssVersion=$(curl -s https://cdn.downloads.dataiku.com/public/dss/ | grep -oP '(?<=href=")[^"]+' | grep -E '^([0-9]+\.)+[0-9]+/$' | sort -V | grep -ie "13.*" | tail -n 1 | sed 's|/$||')

# Debug - DSS (v13) Latest version
echo "$DssVersion"
...
curl -sS "https://cdn.downloads.dataiku.com/public/dss/$DssVersion/dataiku-dss-$DssVersion.tar.gz" -o "/tmp/dataiku-dss-13.tar.gz"

mkdir -p "/opt/dataiku"
mkdir -p "/opt/dataiku/dss_data"

tar -xzf "/tmp/dataiku-dss-13.tar.gz" -C "/opt/dataiku"
