...
yum install -y abrt abrt-cli blktrace cloud-utils-growpart numactl redhat-lsb-core sos sysstat system-config-network-tui time tmpwatch unzip zip
...
if [ -n "$RoleName" ]; then
	...
	NewestAmiInfo=$(aws ec2 describe-images --owner "131827586825" --filter "Name=name,Values=OL6.*" "Name=virtualization-type,Values=hvm" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)|[0]' --output json --region ${Region})
