...
apt install -y -q 7zip acpid acpitool aria2 arptables atop bash-completion bcc bcftools binutils blktrace bpfcc-tools byobu bzip2 chrony collectd collectl colordiff colorized-logs crash crudini cryptol curl dateutils debian-goodies dstat ebtables ethtool expect file fio fping fzf gdisk git glances glances hardinfo hdparm htop httping iftop inotify-tools intltool iotop ipcalc iperf3 iptraf-ng ipv6calc ipv6toolkit jc jnettop jp kexec-tools lsb-release lsof lvm2 lzop manpages mc mdadm mlocate moreutils mtr ncdu ncompress needrestart netcat netsniff-ng nftables nkf nload nmap numactl numatop nvme-cli parted psmisc python3-bpfcc rsync rsyncrypto screen secure-delete shellcheck snmp sosreport ssh-audit strace stressapptest symlinks sysfsutils sysstat tcpdump time timelimit traceroute tree tzdata unzip usermode util-linux wdiff wget zip zstd
