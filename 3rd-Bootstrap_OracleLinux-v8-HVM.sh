...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/refs/heads/main/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_OracleLinux-v8-HVM.json"
...
# DNF-Module Enable Python 3.9 Runtime (from Oracle Linux Repository)
# dnf module list | grep python3
...
# dnf --enablerepo="ol8_developer_EPEL" install -y aria2 atop bash-color-prompt bcftools bpytop byobu collectd collectd-utils colordiff crudini dateutils fping glances htop httping iftop inotify-tools inxi ipv6calc ipv6toolkit java-latest-openjdk moreutils moreutils-parallel ncdu nload rpmconf screen srm ssldump sslscan stressapptest tcping unicornscan vnstat wdiff xq yamllint

dnf --enablerepo="ol8_developer_EPEL" install -y aria2 atop bash-color-prompt bcftools bpytop byobu collectd collectd-utils colordiff crudini dateutils fping glances htop httping iftop inotify-tools inxi ipv6calc ipv6toolkit java-latest-openjdk moreutils moreutils-parallel ncdu nload rpmconf screen srm ssldump sslscan stressapptest tcping unicornscan vnstat wdiff yamllint
...
# 	LatestAmiId=$(aws ec2 describe-images --owner "131827586825" --filter "Name=name,Values=OL8.*" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=x86_64" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)' --output json --region ${Region} | jq -r '[.[] | select(contains({Name: "BETA"}) | not)] | .[0].ImageId')
