...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/master/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_CentOS-v8-HVM.json"
...
# DNF-Module Enable Python 3.6 Runtime (from CentOS Community Repository)
...
# DNF-Module Enable Python 3.9 Runtime (from CentOS Community Repository)
# dnf module list | grep python3
...
if [ -n "$RoleName" ]; then
	...
	NewestAmiInfo=$(aws ec2 describe-images --owner "125523088429" --filter "Name=name,Values=CentOS Stream 8*" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=x86_64" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)|[0]' --output json --region ${Region})
