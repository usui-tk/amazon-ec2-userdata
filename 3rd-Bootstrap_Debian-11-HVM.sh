...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/refs/heads/main/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_Debian-11-HVM.json"
...
apt install -y -q acpid acpitool aria2 arptables atop bash-completion bcc bcftools binutils blktrace bpfcc-tools byobu bzip2 chrony collectd collectd-utils collectl colordiff colorized-logs crash crudini cryptol curl dateutils debian-goodies dstat ebtables ethtool expect file fio fping fzf gdisk git git-lfs hardinfo hdparm htop httping icu-devtools iftop inotify-tools intltool inxi iotop ipcalc iperf3 iptraf-ng ipv6calc ipv6toolkit jc jnettop jq kexec-tools libicu-dev locales-all lsb-release lsof lvm2 lzop manpages mc mdadm mlocate moreutils mtr ncdu ncompress needrestart netcat netsniff-ng nftables nkf nload nmap numactl numatop nvme-cli parted psmisc python3-bpfcc rsync screen secure-delete shellcheck snmp sosreport ssh-audit sslscan strace stressapptest symlinks sysfsutils sysstat tcpdump time timelimit traceroute tree tzdata unzip usermode util-linux vnstat wdiff wget zip zstd
