...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/master/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_RockyLinux-v8-HVM.json"
...
#-------------------------------------------------------------------------------
# Yum Repositories (Rocky Linux 8)
#  https://download.rockylinux.org/pub/rocky/8/BaseOS/x86_64/
#-------------------------------------------------------------------------------
...
# Package Install Rocky Linux Linux-Kernel Modules (from Rocky Linux Community Repository)
dnf install -y kernel-modules kernel-modules-extra

# Package Install Rocky Linux System Administration Tools (from Rocky Linux Community Repository)
...
# Package Install Rocky Linux support tools (from Rocky Linux Community Repository)
...
# Package Install Rocky Linux kernel live-patching tools (from Rocky Linux Community Repository)
...
# Package Install Rocky Linux Web-Based support tools (from Rocky Linux Community Repository)
...
# DNF-Module Enable Python 3.6 Runtime (from Rocky Linux Community Repository)
dnf module list | grep python3
dnf install -y @python36
dnf module list | grep python3

# Package Install Python 3.6 Runtime (from Rocky Linux Community Repository)
...
# DNF-Module Enable Python 3.9 Runtime (from Rocky Linux Community Repository)
# dnf module list | grep python3
# dnf install -y @python39
# dnf module list | grep python3

# Package Install Python 3.9 Runtime (from Rocky Linux Community Repository)
...
dnf --enablerepo="epel" install -y atop bcftools bpytop byobu collectd collectd-utils colordiff dateutils fping glances htop httping iftop inotify-tools ipv6calc ncdu nload screen srm stressapptest tcping yamllint zstd
...
# Package Install Rocky Linux System Administration Tools (from EPEL-Playground Repository)
...
if [ -n "$RoleName" ]; then
	...
	NewestAmiInfo=$(aws ec2 describe-images --owner "679593333241" --filter "Name=name,Values=Rocky Linux 8.*" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=x86_64" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)|[0]' --output json --region ${Region})
...
# Package Install Tuned (from Rocky Linux Community Repository)
