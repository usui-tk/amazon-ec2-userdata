...
# Parameter Settings
CWAgentConfig="https://raw.githubusercontent.com/usui-tk/amazon-ec2-userdata/master/Config_AmazonCloudWatchAgent/AmazonCloudWatchAgent_OracleLinux-v10-HVM.json"
...
#    https://yum.oracle.com/oracle-linux-10.html
...
#-------------------------------------------------------------------------------
# Yum Repositories (Oracle Linux v10)
#  https://yum.oracle.com/oracle-linux-10.html
#-------------------------------------------------------------------------------
...
dnf config-manager --set-enabled ol10_UEKR8
...
# https://docs.oracle.com/en/operating-systems/uek/index.html

if [ $(grubby --default-kernel | grep -ie "el10uek") ]; then
...
# Package Install Python 3.12 Runtime (from Oracle Linux Repository)
...
dnf --enablerepo="ol10_u0_developer_EPEL" install -y dnf --enablerepo="epel" install -y 7zip-standalone-all aria2 colordiff colorized-logs crudini fping htop iftop inotify-tools ipv6calc ipv6toolkit jc lsb_release moreutils moreutils-parallel ncdu nkf nload rpmconf rpmconf screen ssh-audit stressapptest wdiff

# dnf --enablerepo="ol10_u0_developer_EPEL" install -y 7zip-standalone-all aria2 atop byobu collectd collectd-utils colordiff colorized-logs crudini dateutils fping glances htop iftop inotify-tools inxi ipv6calc ipv6toolkit jc lsb_release moreutils moreutils-parallel ncdu nkf nload rpmconf rpmconf screen ssh-audit stressapptest unicornscan wdiff yamllint
...
if [ -n "$RoleName" ]; then
	...
	LatestAmiId=$(aws ec2 describe-images --owner "131827586825" --filter "Name=name,Values=OL10.*" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=x86_64" --query 'sort_by(Images[].{YMD:CreationDate,Name:Name,ImageId:ImageId},&YMD)|reverse(@)' --output json --region ${Region} | jq -r '[.[] | select(contains({Name: "BETA"}) | not)] | .[0].ImageId')
